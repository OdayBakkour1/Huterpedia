...
serve(async (req) => {
  ...
  if (!article || !article.id) {
    return new Response(JSON.stringify({ error: 'Missing article data' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  }
  const { error } = await supabase
    ...
    .insert({
      ...
      cached_content_url: article.cached_content_url || null,
      cached_image_url: article.cached_image_url || null,
      cache_updated_at: article.cache_updated_at || null,
    });
  if (error) {
